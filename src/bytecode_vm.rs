                        ...
                        let lane_results: Vec<Vec<String>> = std::thread::scope(|scope| {
                            let handles: Vec<_> = fork_targets.iter().zip(reg_snaps.into_iter())
                                .map(|(&target_ip, lane_regs)| {
                                    scope.spawn(move || {
                                        run_fork_lane(module, target_ip, lane_regs)
                                    })
                                })
                                .collect();
                            handles.into_iter()
...
fn run_fork_lane(module: &Module, entry_ip: usize, mut regs: Box<[BVal; 256]>) -> Vec<String> {
    ...
    while ip < code_len {
        ...
        match instr.op {
            ...
            // Oracle call (sync — lanes use inline execution, not async engine)
            Op::OracleCall | Op::OracleFused => {
                let oi       = instr.b as usize;
                let args_base = instr.c as usize;
                let n_args   = (instr.flags & 0x0F) as usize;
                let oracle_entries: Vec<usize> =
                    module.oracles.iter().map(|o| o.entry_ip).collect();
                let entry = oracle_entries.get(oi).copied().unwrap_or(0);
                let args: Vec<f64> = (0..n_args)
                    .map(|i| regs[(args_base + i).min(255)].as_f64())
                    .collect();
                let result = exec_oracle_sync(module, entry, &args).unwrap_or(0.0);
