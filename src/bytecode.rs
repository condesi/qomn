    ...
    fn compile_stmt(&mut self, stmt: &Stmt) -> Option<u16> {
        match stmt {
            ...
            Stmt::If { cond, then_body, else_body } => {
                ...
                for s in then_body {
                    if let Some(ra) = self.compile_stmt(s) {
                        self.emit(Instr::new(Op::Return, ra, 0, 0));
                        then_returned = true;
                ...
                if let Some(eb) = else_body {
                    let js = if !then_returned {
                        Some(self.emit(Instr::new(Op::Jump, 0, 0, 0)))
                    } else { None };
    ...
    fn compile_expr(&mut self, expr: &Expr) -> u16 {
        match expr {
            ...
            Expr::Binary(op, lhs, rhs) => {
                ...
                // Desugar: Ge/Le/Ne are not native opcodes
                // a >= b → !(a < b);  a <= b → !(a > b);  a != b → !(a == b)
                let desugar = matches!(op, BinaryOp::Ge | BinaryOp::Le | BinaryOp::Ne);
                let (inner_op, flip) = match op {
                ...
                if flip {
                    ...
                    self.emit(Instr::new(Op::Not, rn, rc, 0));
                    rn
                } else {
                    rc
                }
...
fn pass_dead_code_elim(module: &mut Module) {
    ...
    while i + 1 < len {
        ...
        if cur.op == Op::StoreVar && next.op == Op::LoadVar && cur.a == next.b {
            // Only elide if no further LOAD_VAR reads the same var_idx
            // between i+2 and the next RETURN/HALT. If there are multiple
            // readers, we must keep the STORE_VAR so var_vals is populated
            // in the JIT when those later LOAD_VARs execute.
            let var_idx = cur.a;
            let has_other_reader = module.code[i+2..].iter()
                .take_while(|instr| instr.op != Op::Return && instr.op != Op::Halt)
                .any(|instr| instr.op == Op::LoadVar && instr.b == var_idx);
            if !has_other_reader {
...
fn pass_nop_strip(module: &mut Module) {
    ...
    let pre_strip_len = module.code.len();  // save BEFORE retain
    module.code.retain(|i| i.op != Op::Nop);
    // Update oracle entry_ips: NOP removal shifts oracle start addresses (use pre-strip len for clamp)
    for oracle in &mut module.oracles {
        oracle.entry_ip = ip_map[oracle.entry_ip.min(pre_strip_len)];
    }
...
pub fn disassemble(module: &Module) -> String {
    ...
    for (ip, instr) in module.code.iter().enumerate() {
        ...
        let line = match instr.op {
            ...
            Op::Jump       => format!("{:04}  JUMP         → {:04}", ip, instr.b),
